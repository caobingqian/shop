...
<body>
<div id="formwrapper">
	<h3 class="add_title1">编辑订单</h3>
	<form action="orders!update?pageNo=${page.pageNo }&pageSize=${page.pageSize }" method="post">
...
<c:if test="${login_user.role.id eq '1'}">
		<div>
			...
			<select name="member" id="memberIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq orders.member.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
...
<c:if test="${login_user.role.id eq '2'}">
		...
		<div>
			...
			<select name="goods" id="goodsIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq orders.goods.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
		<div>
			...
			<input type="text" name="settime" id="settime" value="${orders.settime }" size="8" maxlength="200" /> 
			<br />	
		</div>
		<div class="enter">
