...
<c:if test="${login_user.role.id eq '2'}">
		...
		<div>
			...
			<select name="goods" id="goodsIds" >
				...
				<option value="${s.id }"  <c:if test="${s.id eq comments.goods.id }">selected="selected"</c:if>
				>${s.name}</option>
				</c:forEach>
			</select> 
		</div>
		<div>
			...
			<textarea rows="10" cols="100" name="content" id="content" >${comments.content }</textarea>
			</div>
			<br />	
		</div>
		<div>
			<label for="attachs">插入评论内容附件或图片</label>
